...
class Puzzle(object):
    def __init__(self, dictionary: Trie, sides: str, delim: str = "-") -> None:
    ...
    def find_all_words(
        self, starting_with: str = None, current_side_idx: int = 0
    ) -> list[str]:
    ...
    def map_valid_words(self) -> dict:
        ...
        for l in self.all_letters:
            valid_words_map[l] = list(
                filter(lambda x: x[0] == l, self.all_valid_words.copy())
            )
    ...
    def find_solutions(
        ...
        used_letters: set[str] = None,
    ) -> list[list[str]]:
        # Initial word (head of recursion stack):
        if candidate == None:
            solutions = []
            for w in self.all_valid_words:
                solutions += self.find_solutions(max_length, [w], set(w))
            return solutions
        # Recursive call:
        else:
...
def main(**kwargs):
    ...
    if "--output" in kwargs:
        ...
        show_output = (
            input(
                "\nOutput file not specified. Output all solutions to the terminal? (y/N): "
            )
            or "n"
        )
        while show_output.lower() not in {"y", "n"}:
            show_output = (
                input("Output all solutions to the terminal? (y/N): ") or "n"
            )
